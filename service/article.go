...
func GetAllArticle(userid int) (articleInfoList []*model.ArticleInfo, err error) {
	rows, err := dal.DB.Model(&model.ArticleInfo{}).Where("author_id = ?", userid).Rows()
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func GetAllArticleByPage(userid int, pageIndex int, pageSize int) (articleInfoList []*model.ArticleInfo, err error) {
	rows, err := dal.DB.Model(&model.ArticleInfo{}).Where("author_id = ?", userid).Offset((pageIndex - 1) * pageSize).Limit(pageSize).Rows()
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func GiveLikeByArticleId(userid string, strArticleId string, giveLikeTime int64) (err error) {
	//文章点赞总数加1
	if err = dal.DB.Model(&model.ArticleInfo{}).Where("id = ?", strArticleId).Update("give_like_count", gorm.Expr("give_like_count + ?", 1)).Error; err != nil {
		return
	}
...
func IncrCommentCount(articleId int) (err error) {
	err = dal.DB.Model(&model.ArticleInfo{}).Where("id=?", articleId).Update("comment_count", gorm.Expr("comment_count + ?", 1)).Error
	return
}
