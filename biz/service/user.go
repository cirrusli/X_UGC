...
func CheckAccount(account string) (user *model.User, err error) {
	user = new(model.User)
	if err = mysql.DB.Where("number=? or email=?", account, account).First(user).Error; err != nil {
		return nil, err
	}
	return
}
...
func CheckAccountAndPwd(account string, pwd string) (user *model.User, err error) {
	user = new(model.User)
	if err = mysql.DB.Where("(number=? or email=?) and password=?", account, account, pwd).First(user).Error; err != nil {
		return nil, err
	}
	return
}
...
func UpdateUserAccount(userid int, accountType string, account string) (err error) {
	err = mysql.DB.Model(&model.User{}).Where("id = ?", userid).Update(accountType, account).Error
	return
}

// UpdateUserPassword 更新用户密码
func UpdateUserPassword(userid int, newPassword string) (err error) {
	err = mysql.DB.Model(&model.User{}).Where("id = ?", userid).Update("password", newPassword).Error
	return
}

// DeleteAUser 删除一个用户
func DeleteAUser(id string) (err error) {
	err = mysql.DB.Where("id=?", id).Delete(&model.User{}).Error
	return
}
...
func GetAUserInfoByUserId(userid int) (userInfo *model.UserInfo, err error) {
	...
	if err = mysql.DB.Select("*").Where("user_infos.user_id=?", userid).
		Joins("inner join follow_fans_counts on user_infos.user_id=follow_fans_counts.user_id").
...
func GetAUserNameByUserId(userid int) (userName string, err error) {
	...
	if err = mysql.DB.Select("user_name").Where("user_id=?", userid).First(&userinfo).Error; err != nil {
		return "", err
	}
...
func UpdateHeadPhoto(filePath string, id int) (err error) {
	err = mysql.DB.Model(&model.UserInfo{}).Where("user_id=?", id).Update("head_photo", filePath).Error
	return
}

// UpdateBackPhoto  更新背景图片信息
func UpdateBackPhoto(filePath string, id int) (err error) {
	err = mysql.DB.Model(&model.UserInfo{}).Where("user_id=?", id).Update("back_photo", filePath).Error
	return
}
