...
func AddFollowCount(userid int) (err error) {
	err = dal.DB.Create(&model.FollowFansCount{
		UserID: userid,
	}).Error
	return
}
...
func IncrFollowCount(userid string) (err error) {
	err = dal.DB.Model(&model.FollowFansCount{}).Where("user_id=?", userid).Update("follow_count", gorm.Expr("follow_count + ?", 1)).Error
	return
}

// IncrFansCount 粉丝总数+1
func IncrFansCount(userid string) (err error) {
	err = dal.DB.Model(&model.FollowFansCount{}).Where("user_id=?", userid).Update("fans_count", gorm.Expr("fans_count + ?", 1)).Error
	return
}
...
func GetFollowFansCount(userid string) (FollowFansCount *model.FollowFansCount, err error) {
	...
	if err = dal.DB.Where("user_id=?", userid).First(FollowFansCount).Error; err != nil {
		return nil, err
	}
	return
}
